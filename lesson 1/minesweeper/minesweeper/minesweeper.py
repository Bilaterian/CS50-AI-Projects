...
class MinesweeperAI():
        ...
        while newKnowledge:
            ...
            for sentenceA in self.knowledge:
                for sentenceB in self.knowledge:
                    ...
                    if sentenceA.cells == set() and sentenceA.count > 0:
                        self.removeEmpty()

                    # Create a new sentence if 1 is subset of 2, and not in KB:
                    if sentenceA.cells.issubset(sentenceB.cells):
                        new_sentence = Sentence(sentenceB.cells - sentenceA.cells, sentenceB.count - sentenceA.count)

                        # Add to knowledge if not already in KB:
                        if new_sentence not in self.knowledge and (sentenceB.cells - sentenceA.cells) != set() and (sentenceB.count - sentenceA.count) != 0:
                            newKnowledge = True
    ...
    def make_random_move(self):
        ...
        numMinesRemaining = MINES - len(self.mines)
        numSpacesLeft = (self.height * self.width) - (len(self.moves_made) + len(self.mines))

        if numSpacesLeft == 0:
            return None
        
        probabilityOfMine = numMinesRemaining/numSpacesLeft

        for row in range(0, self.height):
            for column in range(0, self.width):
                if (row, column) not in self.moves_made and (row, column) not in self.mines:
                    moves[(row, column)] = probabilityOfMine
